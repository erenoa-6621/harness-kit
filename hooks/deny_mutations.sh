...
#   `sed -i` も `git commit` も `rm` も `> file` も実行できる。分離は**散文の規律**であって、強制ではなかった。
#   公式ドキュメントも、Claude の判断に関わらず行動を止めたいなら PreToolUse フック（exit 2）を使えと示している。
#
# 位置づけ（誇張しない）:
...
def _mktemp_is_default_tmp(argstr):
    ...
    # R6-b（2026-09-03・検証官の R4）: **mktemp の行き先は TMPDIR で変わる。**
...
#   ただし **中身が素のパスなら残す**（2026-09-02 検証官の実測）。
...
def _strip_quoted(text):
    """引用符で囲まれた literal を落とす（素のパスだけは残す）。R5 で関数に切り出した。"""
    ...
    while i < n:
        ...
        if ch in ("'", '"'):
            ...
            i += 1
            lit = "".join(buf)
            stripped.append(lit if _keep_literal(lit) else " ")   # パスでない literal は空白1つに潰す
        else:
...
# ③ 読み取り専用と分かっているサブコマンドは通す（監査官が点検に使う）
#   R4-d: `git stash list|show` を追加。stash の他のサブコマンドは後段の「git 変更系」で止まる。
READ_ONLY = [
    r'\bcrontab\s+-l\b',
    # R6（2026-09-03）: is-system-running / is-enabled / is-failed / list-timers を追加。
    ...
    # R8 (2026-09-03): git merge-base は完全な読み取りの問い合わせだが、
...
def _clean_shell(t):
    """シェル構文の判定に掛ける前の掃除。R5 で関数に切り出した（subprocess の起動先にも同じ掃除を掛けるため）。"""
    # ①-b シェルコメント（行頭または空白の後の # から行末）と算術式 ((…)) は構文判定から外す（R4-a）。
...
#   検証官・監査官は中間結果の保存に一時ファイルが要る。実際、監査官は本日
#   「curl -o でスクラッチパッドに5本書いた。指示に抵触した可能性がある」と自己申告した。
#   **仕事に要るものを禁じると、規律を破るか任務を落とすかの二択になる。** どちらも悪い。
#   /tmp と セッションのスクラッチパッド配下だけを許す。リポジトリ配下は許さない。
TMP_OK = r'(/tmp/|\$TMPDIR|\${TMPDIR)'
def _tmp_ok(t):
    """R6-b（2026-09-03・検証官の R5）: リダイレクト先の判定にも `..` 拒否を掛ける。
...
# ─────────────────────────────────────────────────────────
# 【2026-09-02 全面改訂】検証官が過検知を4件実測した。
...
#   コマンド引数にも適用する。
# ─────────────────────────────────────────────────────────
# R6-b（2026-09-03・検証官の R5）: **`..` を含むパスを一時領域と認めない。**
...
# 2026-09-02、これを区別せず「全引数が /tmp なら通す」としたため、
# `cp /etc/hosts /tmp/.../h` が止まった（読み元 /etc のせい）。
# **読むだけの引数を、書き先として数えない。**
_LAST_ARG_IS_TARGET = {"cp", "mv", "ln", "install"}

# R6（2026-09-03）: chmod / chown は**第1引数がパスではない**。
...
_MUT_PATTERNS = [
    ...
    # R8: merge の直後にハイフンが続く形（merge-base / merge-tree）を変更系と数えない
...
# R8 (2026-09-03): sed -i の書き先を見ていなかった。/tmp の写しへの sed -i まで止まり、
...
def _shell_hits(c):
    ...
    R5 で関数に切り出した。subprocess の `shell=True` / `bash -c` の中身にも同じ判定を掛けるため。"""
    hits = []
    SEGS = [x.strip() for x in re.split(r'&&|\|\||[;\n|]', c) if x.strip()]

    # ④ リダイレクト＝実ファイルへの書き込み（一時領域だけなら通す）
    #   R4-a: `>=` がトップレベルに残っていれば bash は「`=` という名のファイルへの書き込み」と解釈する。止める側で正しい。
    if re.search(r'(^|[^0-9&<>])>>?\s*[^\s&(|]', c) and not _tmp_only(c):
        hits.append("リダイレクトによる書き込み")

    # R6（2026-09-03）: `echo` / `printf` の引数は**データであってコマンドではない**。
    ...
    for seg in SEGS:
        ...
        for pat, label in _MUT_PATTERNS:
            if re.search(pat, seg):
                # R8: その場書き換えは、書き先が全部一時領域なら通す（cp/mv と同じ扱い）
...
class _TmpLitRe:                              # R6-b: python 側にも同じ `..` 拒否を掛ける
    @staticmethod
    def match(x):
        if not x:
            return None
...
def _resolve(expr, depth=0):
    ...
    if m:
        ...
        # R6（2026-09-03）: `open(...).read()` は**文字列**であってパスではない。
...
for m in _RECV_RE.finditer(code):
    ...
    # R6（2026-09-03）: `.replace(` は **str.replace が引数2つ、Path.replace は1つ**。
...
#     - どちらでもない（変数などで辿れない）→ **warn を1行出して通す**。R4 で open に入れたのと同じ扱い。
...
#
# R7（2026-09-03・検証官が検証中に2回踏んだ）: 過検知だった。
